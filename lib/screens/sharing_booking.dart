...
class SharingBooking extends StatefulWidget {
  const SharingBooking({super.key});

  @override
  State<SharingBooking> createState() => _SharingBookingState();
}

class _SharingBookingState extends State<SharingBooking> {
  ...
  Future<void> _addLocation() async {
    if (stopLocations.length >= 3) {
      // Limit stops for sharing rides
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Maximum 3 stops allowed for sharing rides'),
  ...
  void _incrementPassenger() {
    if (passengerCount < 6) {
      // Limit passengers for sharing rides
      setState(() {
        passengerCount++;
      });
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        // Sharing ride info banner
        Container(
          padding: EdgeInsets.all(locationPadding),
          decoration: BoxDecoration(
            color: const Color(0xFF6FCF97).withOpacity(0.1),
            borderRadius: BorderRadius.circular(12),
            border: Border.all(color: const Color(0xFF6FCF97), width: 1),
          ),
          child: Row(
            children: [
              Icon(Icons.group, color: const Color(0xFF6FCF97), size: iconSize),
              SizedBox(width: screenWidth * 0.02),
              Expanded(
                child: Text(
                  'Share your ride with others and save money!',
                  style: GoogleFonts.poppins(
        ...
        // Date and Time row (Responsive layout)
        Row(
          children: [
            // Date field
            Expanded(

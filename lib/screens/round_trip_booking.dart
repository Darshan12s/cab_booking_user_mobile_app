// screens/round_trip_booking.dart
...
class _RoundTripBookingState extends State<RoundTripBooking> {
  ...
  DateTime dropOffDate = DateTime.now().add(
  ...
  Future<void> _selectPickUpDate() async {
    ...
    if (picked != null && picked != pickUpDate) {
      setState(() {
        pickUpDate = picked;
        // Auto-adjust drop-off date if it's before pick-up date
        if (dropOffDate.isBefore(picked)) {
  ...
  Future<void> _selectDropOffDate() async {
    final DateTime? picked = await showDatePicker(
      ...
      firstDate: pickUpDate, // Can't be before pick-up date
      lastDate: DateTime.now().add(const Duration(days: 365)),
    );
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        // Pick-up Date, Time, and Passenger count row (Responsive layout)
        isTablet
            ? Row(
                children: [
                  // Date field
        ...
        // Drop-off Date & time section (Round Trip specific)
        Text(
          'Drop-off Date & time',
          style: GoogleFonts.poppins(
            fontSize: titleFontSize,
        ...
        // Drop-off Date and Time row (Responsive layout)
        isTablet
            ? Row(
                children: [
                  // Drop-off Date field
                  Expanded(
                    ...
                    child: _buildDateTimeField(
                      ...
                      icon: Icons.calendar_today,
                      text:
                          '${dropOffDate.day.toString().padLeft(2, '0')}-${dropOffDate.month.toString().padLeft(2, '0')}-${dropOffDate.year}',
                  ...
                  Expanded(
                    ...
                    child: _buildDateTimeField(
                      ...
                      isTablet,
                      icon: Icons.access_time,
                      text: dropOffTime.format(context),
                      onTap: _selectDropOffTime,
                      fontSize: dateTimeFontSize,
                      iconSize: iconSize,
                    ),
                  ),
                  // Spacer to align with pick-up section
        ...
        // Passenger selector (Responsive layout)
        isTablet
            ? Row(
                children: [
                  PassengerSelector(
